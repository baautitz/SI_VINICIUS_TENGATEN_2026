...
async function waitForContainerHealthy(containerId, timeoutMs = 60000) {
...
const commands = {
  'dev:up': async (remainingArgs = []) => {
    ...
    try {
      ...
      if (!containerId) {
        throw new Error('Container postgres não encontrado');
      }

      await waitForContainerHealthy(containerId);
    } catch (err) {
    ...
    const prefixOutput = (name, data, colorCode) => {
      ...
      for (const line of lines) {
        if (line.trim()) {
          console.log(
            `\x1b[${colorCode}m[${name}]\x1b[0m ${line}`
          );
        }
      }
    };
    ...
    backend.stdout.on(
      'data',
      data => prefixOutput('Backend', data, '36')
    );

    backend.stderr.on(
      'data',
      data => prefixOutput('Backend', data, '31')
    );
    ...
    frontend.stdout.on(
      'data',
      data => prefixOutput('Frontend', data, '35')
    );

    frontend.stderr.on(
      'data',
      data => prefixOutput('Frontend', data, '31')
    );
  ...
  'dev:down': (remainingArgs = []) => {
    process.exit(
      runDockerCompose(['down', ...remainingArgs])
    );
  },
...
if (command === 'help') {
  console.log(
    'Uso: node infra.js [dev:up|dev:down|prod:up|prod:down|<docker-compose-args>]'
  );

  console.log(
    'Para rodar comandos docker compose específicos em produção, use prod:<comando> (ex: prod:ps, prod:logs)'
  );
...
(async () => {
  if (commands[command]) {
    ...
    process.exit(
      runDockerCompose(composeArgs, true)
    );
    ...
    process.exit(
      runDockerCompose(composeArgs, false)
    );
  } else {
    process.exit(
      runDockerCompose(args, false)
    );
  }
})();