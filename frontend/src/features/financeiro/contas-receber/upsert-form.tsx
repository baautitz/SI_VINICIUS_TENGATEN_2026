...
export function ContasReceberUpsertForm({
  ...
  const [parcelas, setParcelas] = useState<ContasReceberParcela[]>(
    editingItem?.contasReceberParcelas?.map((p) => ({
  ...
  const {
    ...
    mutationFn: async (value: ContasReceberFormValues) => {
      const payload = {
        ...value,
      ...
      return isEditMode
        ? await contasReceberApi.update(editingItem.id, payload)
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      const validationResult = contasReceberSchema.safeParse(payload);
  ...
  const gerarSugeridas = (
    ...
    const sugeridas: ContasReceberParcela[] = [];
    let count = 1;
  ...
  const handleUpdateParcela = (
    index: number,
    key: keyof ContasReceberParcela,
    value: string | number
  ) => {
    if (readOnly) return;
  ...
  return (
    <UpsertDialog
      ...
      title={isEditMode ? "Detalhes da Conta a Receber" : "Nova Conta a Receber"}
      footer={
        <>
          <DialogClose asChild>
        ...
        <div className="flex w-full flex-col gap-6">
          <div className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <form.Field
                name="descricao"
                validators={{ onChange: contasReceberBaseSchema.shape.descricao }}
              >
                {(field) => (
              ...
              <form.Field
                name="clienteId"
                validators={{ onChange: contasReceberBaseSchema.shape.clienteId }}
              >
                {(field) => {
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <form.Field
                name="valorOriginal"
                validators={{ onChange: contasReceberBaseSchema.shape.valorOriginal }}
              >
                {(field) => (
          ...
          <div className="flex flex-col gap-4">
            <div className="flex justify-between items-center">
              <h3 className="text-sm font-semibold tracking-wider">Parcelas de Recebimento</h3>
              {!readOnly && (
                        ...
                        <Table>
                          <TableHeader>
                            <TableRow className="hover:bg-transparent">
                              ...
                              <TableHead className="px-2 text-right">Valor Recebido (R$)</TableHead>
