...
import type { Transportadora, TransportadoraResumo, TransportadoraFormValues } from "@/features/logistica/transportadoras/types";
import type { Veiculo, VeiculoResumo, VeiculoFormValues } from "@/features/logistica/veiculos/types";

export const transportadorasApi = {
  list: (search?: string, page = 1, pageSize = 20) =>
    http.get<PaginatedResult<TransportadoraResumo>>(`/api/logistica/transportadoras?search=${encodeURIComponent(search ?? "")}&page=${page}&pageSize=${pageSize}`),
...
export const veiculosApi = {
  list: (search?: string, page = 1, pageSize = 20) =>
    http.get<PaginatedResult<VeiculoResumo>>(`/api/logistica/veiculos?search=${encodeURIComponent(search ?? "")}&page=${page}&pageSize=${pageSize}`),
