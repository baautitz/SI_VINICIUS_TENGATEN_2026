...
import type { Cliente, ClienteResumo, ClienteFormValues } from "@/features/parceiros/clientes/types";
import type { Emitente, EmitenteResumo, EmitenteFormValues } from "@/features/parceiros/emitentes/types";
import type { Fornecedor, FornecedorResumo, FornecedorFormValues } from "@/features/parceiros/fornecedores/types";

export const clientesApi = {
  list: (search?: string, page = 1, pageSize = 20) =>
    http.get<PaginatedResult<ClienteResumo>>(`/api/parceiros/clientes?search=${encodeURIComponent(search ?? "")}&page=${page}&pageSize=${pageSize}`),
...
export const emitentesApi = {
  list: (search?: string, page = 1, pageSize = 20) =>
    http.get<PaginatedResult<EmitenteResumo>>(`/api/parceiros/emitentes?search=${encodeURIComponent(search ?? "")}&page=${page}&pageSize=${pageSize}`),
...
export const fornecedoresApi = {
  list: (search?: string, page = 1, pageSize = 20) =>
    http.get<PaginatedResult<FornecedorResumo>>(`/api/parceiros/fornecedores?search=${encodeURIComponent(search ?? "")}&page=${page}&pageSize=${pageSize}`),
